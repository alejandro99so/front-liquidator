...
import { bucksGet } from "../../utils/fetchWithToken";
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      ...
      try {
        ...
        const data: TrxRequest[] = await bucksGet("room/request?active=true");
        console.log({ data });
            ...
            <div className={styles.info}>
              <div className={styles.user}>
                ...
                <span className={styles.address}>
                  {formatWalletAddress(trx.userAddress)}
                </span>
              </div>
              <span className={styles.rate}>
                rate: {rate}.00 / {trx.usd} USD
              </span>
              <span className={styles.amount}>{trx.cop}.00 COP </span>
            </div>
            <div className={styles.details}>
              <div className={styles.containerButtonDetails}>
                <Link href={`/Details/${trx._id}`} className={styles.link}>
                  Details
                </Link>
              </div>
              <div className={styles.logos}>
                <span className={styles.border}>
                  <Image
                    src={`/icons/${trx.cryptoCurrency}.svg`}
                    alt={`${trx.cryptoCurrency}.svg`}
                    width={20}
                    height={20}
                  />
                </span>
                <span className={styles.border}>
                  <Image
                    src={`/icons/${networks[trx.network]}.png`}
                    alt={`${networks[trx.network]}.png`}
                    width={20}
                    height={20}
                  />
                </span>
